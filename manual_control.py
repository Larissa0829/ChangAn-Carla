...
class World(object):
    def __init__(self, carla_world, hud, args):
        self.world = carla_world  # 获取世界
        self.sync = args.sync  # 获取是否异步
        self.actor_role_name = args.rolename  # 获取角色名字
        try:
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = Transform(Location(x=-48.567417, y=102.479195, z=0.600000),
                                    Rotation(pitch=0.000000, yaw=89.838760,
                                             roll=0.000000)) if spawn_points else carla.Transform()
            print("spawn_point:",
                  spawn_point)  # spawn_point: Transform(Location(x=32.992611, y=-67.899994, z=0.600000), Rotation(pitch=0.000000, yaw=179.976562, roll=0.000000))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.show_vehicle_telemetry = False
            self.modify_vehicle_physics(self.player)
